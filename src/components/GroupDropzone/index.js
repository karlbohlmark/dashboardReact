...
function GroupDropzone(props) {
    ...
    function handleModalClickOk() {
        ...
        const theFile = Just(toBlob(cropperRef.getCroppedCanvas().toDataURL('image/png')));
        theFile.cata({
            Nothing: noop,
            Just: props.onUpload
        });
    ...
    return (
        <div>
            <Modal show={props.modal.cata({
                ...
                <Modal.Footer>
                    <Button onClick={compose(handleModalClickOk, props.onCloseModal)}>
                        Ok
                    </Button>
                    <Button
                        bsStyle="danger"
            ...
            >
                {props.image.cata({
                    Nothing: () => (
                        <Dropzone
                            onDrop={compose(props.onShowModal, props.onModalImageAdd, get('0'))}
                                // head(files).cata({
                                //     Nothing: noop,
                                //     Just: props.onUpload
                                // });
